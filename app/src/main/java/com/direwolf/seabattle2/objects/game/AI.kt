...
import com.direwolf.seabattle2.utils.ShipPlacementLearner

class AI(private val context: Context? = null, private val advancedDifficulty: Boolean = false) {
    ...
    private var enemyShips = intArrayOf(0, 0, 0, 5) // Only single-deck ships (length 1 = index 0)
    private val learner: ShipPlacementLearner? = if (advancedDifficulty && context != null) {
        ShipPlacementLearner(context)
    } else null
    fun setShips(): Array<Int> {
        var coords: Array<Int>
        // Only 5 single-deck ships
        while (true) {
            coords = emptyArray()
            for (n in 0 until 5) {
                val res = setShip(1)
                coords += res[0] as Int
                coords += res[1] as Int
                coords += 0  // length - 1 (since length is 1)
    ...
    private fun setCoef(): Array<Array<Int>> {
        ...
        // Advanced difficulty: use learned patterns
        if (advancedDifficulty && learner != null) {
            val hotspots = learner.getPredictedHotspots()
            for (x in 0..9) {
                for (y in 0..9) {
                    if (enemyField[x][y] == 0) {
                        coef[x][y] += hotspots[x][y] * 10 // Boost probability based on learned patterns
                    }
                }
            }
        }
        ...
        for (type in 1..4){
            if (type != 1 || enemyShips[type - 1] <= 0){ // Only check for single-deck ships
                continue
            }
    ...
    fun recordPlayerPlacement(ships: Array<Int>) {
        learner?.recordPlayerPlacement(ships)
    }
    
    fun learnFromRadar(x: Int, y: Int) {
        // Boost probability in this area
        if (x in 0..9 && y in 0..9 && enemyField[x][y] == 0) {
            // This will be handled in setCoef by checking hotspots
        }
    }
