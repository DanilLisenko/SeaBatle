...
class AIGrid(private val context: Context, private val layout: ConstraintLayout,
    ...
    private fun createBoom(isShip: Boolean, x: Int, y: Int) {
        ...
        if (isShip) {
            // Получаем View из ячейки
            val cellView = cells[x][y].getView()

            // Вместо иконки ship_hit ставим просто красный фон
            val background = GradientDrawable()
            background.setColor(android.graphics.Color.RED)
            background.setStroke(2, android.graphics.Color.WHITE) // Тонкая рамка, чтобы клетки не сливались

            cellView.background = background

            // Если это ImageView, убираем картинку (на случай если она там была)
            if (cellView is ImageView) {
                cellView.setImageResource(0)
            }
        } else {
            // Логика промаха (синяя точка) остается без изменений
